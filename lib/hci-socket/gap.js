...
Gap.prototype.onHciLeAdvertisingReport = function(status, type, address, addressType, eir, rssi) {
  ...
  discoveryCount++;
  var connectable = (type === 0x00) || (type === 0x04);

  if (type === 0x04) {
    hasScanResponse = true;
    this.emit('scan_response', { status, address, rssi, connectable, eir });
  } else {
    this.emit('raw_discover', { status, address, rssi, connectable, eir });
  }
